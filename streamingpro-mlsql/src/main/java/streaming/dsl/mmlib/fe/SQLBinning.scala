...
import streaming.dsl.mmlib.algs.components.ScoreCard
...
object Hello {
  def findOutStrCols(df: DataFrame): Array[String] = {
    df.columns
      .map(c => {
        df.select(c).limit(1).dtypes(0)
      }).filter(_._2 == StringType.toString)
  ...
  def getIndexer(labels: Seq[String], labelToIndex: mutable.HashMap[String, Double]) = {
  ...
  def main(args: Array[String]): Unit = {
    val spark = SparkSession
      ...
      .getOrCreate()
    val trainning_data = spark.createDataFrame(Seq(
      ("elena", 57, 15552231521L, 433000, 0),
      ("candy", 67, 15552231521L, 190000, 0),
      ("bob", 57, 15252211521L, 89000, 0),
      ("candy", 25, 15552211522L, 36000, 1),
      ("candy", 31, 15552211521L, 299000, 1),
      ("finn", 23, 15552211521L, 238000, 1),
      ("gina", 59, 15252211521L, 153000, 0),
      ("gina", 64, 15252211521L, 165000, 0),
      ("elena", 66, 15552211522L, 152000, 0),
      ("andie", 49, 15252211521L, 38000, 1),
      ("bob", 60, 15552211521L, 95000, 0),
      ("finn", 65, 15552211521L, 184000, 0),
      ("gina", 42, 15552211522L, 338000, 1),
      ("bob", 48, 15552231521L, 146000, 1),
      ("gina", 35, 15252211521L, 96000, 1),
      ("gina", 43, 15552231521L, 374000, 1),
      ("elena", 58, 15552211521L, 170000, 0),
      ("candy", 57, 15552231521L, 388000, 0),
      ("bob", 50, 15552211521L, 243000, 1),
      ("finn", 57, 15252211521L, 135000, 0)
    )) toDF("name", "age", "phone", "income", "label")

    val df1 = MLMapping.findAlg("Binning").train(trainning_data,
      "/tmp/test2",
      Map(Binning.SELECTED_FEATURES -> "name,age,income",
        Binning.NUM_BUCKETS -> "3",
        Binning.LABEL_COLNAME -> "label",
        Binning.BINNING_METHOD -> "EF"))
    df1.createTempView("binningInfoTable")
    val df2 = MLMapping.findAlg("ScoreCard").train(trainning_data, "/tmp/scorecard0118",
      Map(ScoreCard.BINNING_TABLE -> "binningInfoTable",
        ScoreCard.BINNING_PATH -> "/tmp/test2",
        ScoreCard.SELECTED_FEATURES -> "name,age,income"
      ))
    df2.show()
    val testing_data = spark.createDataFrame(Seq(
      ("gina", 22, 15552211521L, 479000, 1),
      ("gina", 64, 15552211522L, 342000, 0),
      ("elena", 26, 15252211521L, 357000, 1),
      ("gina", 50, 15552211521L, 174000, 0),
      ("elena", 48, 15552211521L, 324000, 1),
      ("andie", 30, 15252211521L, 224000, 1),
      ("elena", 60, 15552211522L, 486000, 0),
      ("finn", 28, 15252211521L, 258000, 1),
      ("bob", 79, 15252211521L, 113000, 0),
      ("gina", 75, 15552231521L, 105000, 0))) toDF("name", "age", "phone", "income", "label")
    //    val test_res = MLMapping.findAlg("ScoreCard").batchPredict(testing_data, "/tmp/scorecard0118",
    //      Map(
    //        ScoreCard.BINNING_PATH -> "/tmp/test2",
    //        ScoreCard.BINNING_TABLE -> "binningInfoTable",
    //        ScoreCard.SELECTED_FEATURES -> "name,age,income"
    //      ))
    //    test_res.show()

    val model = MLMapping.findAlg("ScoreCard").load(testing_data.sparkSession, "/tmp/scorecard0118",
      Map(ScoreCard.BINNING_PATH -> "/tmp/test2",
        ScoreCard.BINNING_TABLE -> "binningInfoTable"))
    val scorecard_udf = MLMapping.findAlg("ScoreCard").predict(testing_data.sparkSession, model, "scorecard", Map())
    spark.udf.register("scorecard", scorecard_udf)
    spark.createDataFrame(Seq(("candy", "68", "125000")))
      .toDF("name", "age", "income")
      .createOrReplaceTempView("t_foo")
    spark.sql("select scorecard(array(name,age,income)) from t_foo").show()
    //    val df1 = MLMapping.findAlg("Binning").batchPredict(, "/tmp/test1", Map())
    //    df1.show()
    //    val model = MLMapping.findAlg("Binning").load(df.sparkSession, "/tmp/test1", Map())
    //    val udf = MLMapping.findAlg("Binning").predict(df.sparkSession, model, "test", Map())
    //    spark.udf.register("test", udf)
    //    spark.createDataFrame(Seq(("hong", "11")))
    //      .toDF("name", "age")
    //      .createOrReplaceTempView("t_foo")
    //    spark.sql("select test(array(name,age)) from t_foo").show()
    //
    //
    //    val df2 = spark.createDataFrame(Seq(
    //      ("hong", 20, 15552211521L),
    //      ("hong", 19, 13287994007L),
    //      ("zhi1", 21, 15552211523L)
    //    )) toDF("name", "age", "phone")
